...
for (const [nr, p] of window.POZIOMY.entries()) {
  ...
  const wolne = (x, y) => !staly(x, y);                                                     // czy ciało może tam być
  const miesci = (x, y) => wolne(x, y - 1) && wolne(x, y - 2);                            // stojąc na y: ciało w y-1 i y-2
  const stoi = (x, y) => y < H && podloze(x, y) && miesci(x, y) && at(x, y) !== 'V';
  const spadnij = (x, y) => { // z powietrza w (x, y-1) spadaj aż do podłoża; null = śmierć/nic
    for (let yy = y; yy < H; yy++) { if (at(x, yy) === 'V') return null; if (podloze(x, yy)) return miesci(x, yy) ? [x, yy] : null; }
    return null;
  };
  let start = null, cele = [];
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
  ...
  const laduj = (x, y) => { if (!miesci(x, y)) return; const s = stoi(x, y) ? [x, y] : spadnij(x, y); if (s) dodaj(...s); };
  while (kolejka.length) {
    const [x, y] = kolejka.shift();
    // chód
    for (const dx of [-1, 1]) { if (miesci(x + dx, y)) laduj(x + dx, y); }
    // skoki: (dy w górę, max odległość w kaflach)
    for (const [up, maxD] of [[2, 3], [1, 4], [0, 4], [-1, 5], [-2, 5], [-3, 6], [-4, 6], [-5, 6], [-6, 6]]) {
      for (const dir of [-1, 1]) for (let d = 1; d <= maxD; d++) {
        ...
        // trasa w powietrzu: wymagamy wolnej przestrzeni nad startem (przybliżenie)
        if (!wolne(x, y - 3) && up > 0) break;
        if (!miesci(tx, ty)) { if (staly(tx, ty - 1) || staly(tx, ty - 2)) break; else continue; }
        if (stoi(tx, ty)) dodaj(tx, ty); else if (up <= 0) { const s = spadnij(tx, ty); if (s) dodaj(...s); }
      }
    }
    // drabiny: środek ciała w (x, y-1)
    if (at(x, y - 1) === 'H' || at(x, y) === 'H') {
      for (let yy = y - 1; yy >= 1; yy--) {
        if (at(x, yy) !== 'H' && at(x, yy + 1) !== 'H') break;
        if (!wolne(x, yy - 1)) break;
        for (const dx of [-1, 1]) if (miesci(x + dx, yy + 1)) laduj(x + dx, yy + 1);
        if (stoi(x, yy + 1) || at(x, yy) === 'H') dodaj(x, yy + 1);
      }
    }
  }
  ...
  console.log(`Poziom ${nr + 1} ${p.nazwa}: start ${start}, meta osiągalna: ${osiagalne.length}/${cele.length}, pozycji: ${odw.size}, przedmioty ${szmOk}/${szm}` + (brak.length ? `  NIEOSIĄGALNE: ${brak.join(' ')}` : ''));
